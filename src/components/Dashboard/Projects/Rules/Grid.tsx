...
const RulesGrid = () => {
  ...
  const typeReverseMapping: any = {
    1: "complex",
    2: "patternmatching (re)",
  };

  const searchData = ruleData.filter((projectItem: any) => {
    const nameMatch = projectItem.name
      .toLowerCase()
      .includes(nameSearch.toLowerCase());
    const typeSearchLower = typeSearch.toLowerCase();
    const typeString = typeReverseMapping[projectItem.type];
    const typeMatch = typeString && typeString.includes(typeSearchLower);
    ...
    return nameMatch && dateMatch && actionMatch && typeMatch;
  });
  ...
  }, [nameSearch, dateSearch, actionSearch, typeSearch]);

  useEffect(() => {
    projectService.getRulesByProject(param.id).then((result) => {
      if (result.count > 0) {
        setRuleData(result.result);
    ...
    <div className="rounded-sm border border-stroke bg-white px-5 pb-2.5 pt-6 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
      <div className="max-w-full overflow-x-auto">
        <table className="w-full table-auto">
          ...
          <tbody>
            {currentData &&
              currentData.map((packageItem: any, key: number) => (
                <tr key={key}>
                  ...
                  <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                    <div className="flex items-center space-x-3.5">
                      ...
                      <Link
                        href={`/projects/manage/${param.id}/rules/edit?rule_id=${packageItem.id}`}
                      >
                        <button className="hover:text-primary">
                          <svg
                            className="fill-current"
