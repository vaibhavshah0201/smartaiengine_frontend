...
const ProjectsPage = () => {
  ...
  const handleSubmit = async (event: any) => {
    ...
    formData.append("project_id", param.id);
    formData.append("description", description);

    const response: any = await fetch("http://localhost:8000/file/add", {
      method: "POST",
      body: formData,
      // headers: {
    ...
    if (response.status == 200) {
      router.push(`/projects/manage/${param.id}/files`);
    }
  };
            ...
            <div className="p-7">
              <form action="POST" onSubmit={handleSubmit}>
                <div className="flex flex-col items-center justify-center h-64">
                  {selectedFile ? (
                    <span className="border p-5">{selectedFile.name}</span>
                  ) : (
                    <div
                      id="FileUpload"
                ...
                <div className="flex justify-end gap-4.5">
                  {selectedFile == null && (
                    <Link href={`/projects/manage/${param.id}/files`}>
                      <button
